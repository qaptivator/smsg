    ...
    <div v-show="tabType === 'main'">
      <div class="m-auto p-5 mt-10">
        <div v-if="!isConnected">
          ...
          <div v-if="this.permsAccepted" class="w-full text-center">
            <button
        ...
        <div v-if="!this.permsAccepted" class="w-full text-center text-red-500">
          Certain permission like receival of push notification or sending SMS
          messages was not accepted. Check app's permissions and restart the
          app.
        </div>
      </div>
      <div class="flex">
...
export default {
  ...
  async mounted() {
    ...
    //await this.requestMessageSendingPermission();
  ...
  methods: {
    ...
    async registerNotifications() {
      ...
      if (permStatus.receive !== 'granted') {
        this.permsAccepted = false;
      }

      await PushNotifications.register();

      this.throwLog('Registered push notifications.', 'info');
    },
    //#endregion
      ...
      if (this.enableMessageQueueInterval) {
        ...
        PushNotifications.addListener(
          ...
          notification => {
            this.getMessageQueue();
            this.throwLog(
              `Push notifications received (id:${notification.id})`,
              'info'
            );
          }
        );
    ...
    sendSMS(recipient, message, id, data) {
      ...
      if (recipient && message) {
        ...
        if (recipient.charAt(0) === '+') {
          ...
          this.throwMessage(`Invalid phone number! (smsid:${id})`, 'warn');
      ...
      sms.hasPermission(
        sendingPerms => {
          if (sendingPerms === true) {
            ...
            this.throwLog(
              'SMS sending permission not found! Check app permissions and restart the app.',
              'warn'
            );
    ...
    getMessageQueue() {
      ...
      if (/*this.messagesQueue.length === 0 &&*/ this.messagesQueueUrl !== '') {
        ...
        axios
          ...
          .then(response => {
            if (response.data.messages) {
              ...
              //if (this.checkMessageSendingPermission()) { temporary solution
    ...
    async connect() {
        ...
        cordova.plugins.SMSReceive.startWatch(
          success => {
            cordova.plugins.SMSReceive.stopWatch(
              ...
              error => {
                this.throwLog(
                  `Connection failed. Try reconnecting. (error:${error})`,
                  'error'
                );
                this.throwStatus(error);
                this.throwStatus('CONNECTION_FAILED');
      ...
      let smsReadingResult = await new Promise((resolve, reject) => {
        cordova.plugins.SMSReceive.startWatch(
          success => {
            cordova.plugins.SMSReceive.stopWatch(
              success => {
      ...
      console.log('SMS reading permission check passed.');
      console.log('All permission checks had passed.');

      this.startMessagesWatcher();
      ...
      /*if (this.isConnected === false && this.validForm && this.permsAccepted) {
        if (this.webhookUrl !== '') {
          this.startMessagesWatcher();
